...
import { Hierarchy } from "../../components/charts/HierarchyTree";

const getHierarchyData = (detailed = false): Hierarchy => {
  const nLevels = detailed ? 8 : 3;
  const nodeStructure = range(0, nLevels).map((d) => randomInt(2, 5)());
...
export default getHierarchyData;
